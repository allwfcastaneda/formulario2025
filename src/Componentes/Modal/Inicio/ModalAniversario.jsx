...
export const ModalAniversario = ({estaModalAbierto, nombre, email, setMensajeExito, cerrarModal}) => {
    ...
    const GuardarSaludo = async () => {
        SetGuardando(true);
        let obj = {
            correoAniversario: email,
            nombreAniversario: nombre.split(" ")[0],
        ...
        let saludoAniversario = await fetchToken({
            endpoint: `${import.meta.env.VITE_API_URL}/inicio/insAniversario`,
            method: "POST",
            token: token,
            body: obj
          });

        // console.log(saludoAniversario);

        if(setMensajeExito && saludoAniversario[0].FlgOk == 1){
            setMensajeExito("Tu saludo de aniversario fue enviado exitosamente al correo entel.");

            CerrarModalAniversario();
        }
    }

    const CerrarModalAniversario = () => {
        if(cerrarModal) {
            SetAltoModal("454px");
            ...
            <DivContenedorModal>
                ...
                <div className="contenedor" style={{display:(preVisualizacion ? "none" : "")}}>
                    ...
                    <div className="card-cumpleanios">
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-1.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-2.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-3.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-4.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-5.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                        <img className="img-cumple" src="https://entel.blob.core.windows.net/cdn/img/mail-soyentel/aniversario-6.png" alt="" onClick={(e) => { SeleccionarImg(e.target); }} />
                    </div>
                    <h4>Escribe tu mensaje de felicitaciones:</h4>
                ...
                {preVisualizacion ?
                    <DivPreview id="cumple-preview">
                        <h4>Preview de tu saludo de Aniversario:</h4>
                        <h5>¡Feliz Aniversario <span id="nombre-cumple-preview">{nombre.split(" ")[0]}</span>!</h5>
